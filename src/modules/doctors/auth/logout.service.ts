...
export class LogoutService {
  ...
  async logout(token: string): Promise<{ message: string }> {
    ...
    logger.info(`Doctor ${userId} logged out from single device`);
  ...
  async logoutAllDevices(token: string): Promise<{ message: string }> {
    ...
    logger.warn(
      `Doctor ${userId} logged out from ALL devices (security action)`,
    );
  ...
  async cleanupExpiredTokens(): Promise<{ deleteCount: number }> {
  ...
  async invalidateAllTokenForUser(
    userId: string,
    reason: "PASSWORD_CHANGED" | "ACCOUNT_DEACTIVATED" | "SECURITY",
  ): Promise<void> {
    await prisma.tokenBlacklist.create({
      data: {
        token: `INVALIDATE_ALL_${userId}_${Date.now()}`,
        userId,
        reason,
        expiresAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000),
      },
    ...
    logger.warn(
      `All tokens invalidated for doctor ${userId}. Reason ${reason}`,
    );
  }
}
