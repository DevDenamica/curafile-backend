...
export class LogoutService {
  ...
  async logout(token: string): Promise<{ message: string }> {
    ...
    logger.info(`User ${userId} logged out from single device`);
  ...
  async logoutAllDevices(token: string): Promise<{ message: string }> {
    ...
    logger.warn(`User ${userId} logged out from ALL devices (security action)`);
  ...
  async invalidateAllTokensForUser(
    userId: string,
    ...
    logger.warn(`All tokens invalidated for user ${userId}. Reason: ${reason}`);
  }
}
